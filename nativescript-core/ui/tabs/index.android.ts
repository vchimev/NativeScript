...
function initializeNativeClasses() {
	...
	class TabFragmentImplementation extends org.nativescript.widgets
		.FragmentBase {
		private owner: Tabs;
		...
		public onDestroyView() {
			...
			if (hasRemovingParent && this.owner.selectedIndex === this.index) {
				...
				this.owner._originalBackground =
					this.owner.backgroundColor || new Color("White");
				this.owner.nativeViewProtected.setBackgroundDrawable(
					bitmapDrawable
				);
	...
	class FragmentPagerAdapter extends androidx.viewpager.widget.PagerAdapter {
		...
		instantiateItem(
			...
			let fragment: androidx.fragment.app.Fragment = fragmentManager.findFragmentByTag(
				name
			);
			if (fragment != null) {
		...
		destroyItem(
			...
			const fragment: androidx.fragment.app.Fragment = <
	...
	class TabsBarImplementation extends org.nativescript.widgets.TabsBar {
		...
		public onSelectedPositionChange(
			position: number,
			prevPosition: number
		): void {
...
function setElevation(
	...
	tabsBar: org.nativescript.widgets.TabsBar,
	tabsPosition: string
) {
...
export class Tabs extends TabsBase {
	...
	public onItemsChanged(
		oldItems: TabContentItem[],
		newItems: TabContentItem[]
	): void {
	...
	public createNativeView() {
		...
		const primaryColor = ad.resources.getPaletteColor(
			PRIMARY_COLOR,
			context
		);
	...
	private disposeCurrentFragments(): void {
		...
		for (let fragment of <Array<any>>(
			fragmentManager.getFragments().toArray()
		)) {
	...
	private createTabItemSpec(
		tabStripItem: TabStripItem
	): org.nativescript.widgets.TabItemSpec {
		...
		if (tabStripItem.isLoaded) {
			...
			tabItemSpec.backgroundColor = backgroundColor
				? backgroundColor.android
				: this.getTabBarBackgroundArgbColor();
	...
	private getFixedSizeIcon(
		...
		const scaledImage = android.graphics.Bitmap.createScaledBitmap(
			image,
			widthPixels,
			heightPixels,
			true
	...
	public updateAndroidItemAt(
		index: number,
		spec: org.nativescript.widgets.TabItemSpec
	) {
	...
	public setTabBarItemFontInternal(
		...
		tabStripItem.nativeViewProtected.setTypeface(
			value.getAndroidTypeface()
		);
	}

	public getTabBarItemTextTransform(
		tabStripItem: TabStripItem
	): TextTransform {
		return this.getItemLabelTextTransform(tabStripItem);
	}

	public setTabBarItemTextTransform(
		tabStripItem: TabStripItem,
		value: TextTransform
	): void {
		const nestedLabel = tabStripItem.label;
