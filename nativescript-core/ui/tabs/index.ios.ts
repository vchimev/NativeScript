...
class BackgroundIndicatorTemplate extends NSObject
	implements MDCTabBarIndicatorTemplate {
	public static ObjCProtocols = [MDCTabBarIndicatorTemplate];

	public indicatorAttributesForContext(
		context: MDCTabBarIndicatorContext
...
class UIPageViewControllerImpl extends UIPageViewController {
	...
	public static initWithOwner(
		...
		const handler = <UIPageViewControllerImpl>(
			UIPageViewControllerImpl.alloc().initWithTransitionStyleNavigationOrientationOptions(
	...
	public viewDidLoad(): void {
		...
		if (majorVersion <= 12 || !UIColor.labelColor) {
			...
			tabBar.setTitleColorForState(
				UIColor.blackColor,
				MDCTabBarItemState.Normal
			);
			tabBar.setTitleColorForState(
				UIColor.blackColor,
	...
	public viewDidLayoutSubviews(): void {
		...
		let scrollViewHeight =
			this.view.bounds.size.height + safeAreaInsetsBottom;
		...
		if (owner.tabStrip) {
			...
			if (tabsPosition === "bottom") {
				...
				scrollViewHeight =
					this.view.frame.size.height - safeAreaInsetsBottom;
			}
			...
			if (parent && majorVersion > 10) {
				...
				tabBarTop = Math.max(
					tabBarTop,
					parent.nativeView.safeAreaInsets.top
				);
			}
	...
	public traitCollectionDidChange(
		...
		if (majorVersion >= 13) {
			...
			if (
				...
				owner.notify({
					eventName:
						iosView.traitCollectionColorAppearanceChangedEvent,
					object: owner,
				});
			}
		}
	...
	public viewWillTransitionToSizeWithTransitionCoordinator(
		...
		super.viewWillTransitionToSizeWithTransitionCoordinator(
			size,
			coordinator
		);
			...
			if (owner && owner.tabStrip && owner.tabStrip.items) {
				...
				tabStrip.items.forEach((tabStripItem) => {
					updateBackgroundPositions(
						tabStrip,
						tabStripItem,
						this.tabBar.alignment !==
							MDCTabBarAlignment.Justified ||
							owner.selectedIndex !== tabStripItem._index
							? owner._defaultItemBackgroundColor
							: null
					);
...
class UIPageViewControllerDataSourceImpl extends NSObject
	...
	public static initWithOwner(
		...
		let dataSource = <UIPageViewControllerDataSourceImpl>(
			UIPageViewControllerDataSourceImpl.new()
		);
	...
	public pageViewControllerViewControllerBeforeViewController(
	...
	public pageViewControllerViewControllerAfterViewController(
		pageViewController: UIPageViewController,
		viewController: UIViewController
	): UIViewController {
		// TODO
	...
	public presentationCountForPageViewController(
		pageViewController: UIPageViewController
	): number {
		// TODO
		// if (Trace.isEnabled()) {
	...
	public presentationIndexForPageViewController(
		pageViewController: UIPageViewController
	): number {
		// TODO
		// if (Trace.isEnabled()) {
...
class UIPageViewControllerDelegateImpl extends NSObject
	...
	public static initWithOwner(
		owner: WeakRef<Tabs>
	): UIPageViewControllerDelegateImpl {
		let delegate = <UIPageViewControllerDelegateImpl>(
			UIPageViewControllerDelegateImpl.new()
		);
		delegate._owner = owner;
	...
	public pageViewControllerWillTransitionToViewControllers(
		pageViewController: UIPageViewController,
	...
	public pageViewControllerDidFinishAnimatingPreviousViewControllersTransitionCompleted(
		...
		const nextViewControllerIndex = ownerViewControllers.indexOf(
			nextViewController
		);
...
function updateTitleAndIconPositions(
	tabStripItem: TabStripItem,
...
export class Tabs extends TabsBase {
	...
	constructor() {
		...
		this.viewController = this._ios = <UIPageViewControllerImpl>(
			UIPageViewControllerImpl.initWithOwner(new WeakRef(this))
	...
	private getViewController(item: TabContentItem): UIViewController {
		let newController: UIViewController = item.content
			? item.content.viewController
			: null;
		...
		if (item.content.ios instanceof UIViewController) {
			...
			newController = iosView.UILayoutViewController.initWithOwner(
			...
			(<any>item).setViewController(
				newController,
				item.content.nativeViewProtected
			);
		}
	...
	private setViewControllers(items: TabContentItem[]) {
		...
		if (tabStripItems) {
			if (tabStripItems[this.selectedIndex]) {
				tabStripItems[this.selectedIndex]._emit(
					TabStripItem.selectEvent
				);
			}
		}
		...
		if (this.viewController && this.viewController.tabBar) {
			...
			this.viewController.tabBar.items = NSArray.arrayWithArray(
				this.tabBarItems
			);
			...
			if (this.selectedIndex) {
				this.viewController.tabBar.setSelectedItemAnimated(
					this.tabBarItems[this.selectedIndex],
					false
				);
			}
		}
			...
			if (this.tabStrip && this.tabStrip.items) {
				this.tabStrip.items.forEach((item) => {
					if (this._unSelectedItemColor && item.nativeView) {
						item.nativeView.image = this.getIcon(
							item,
							this._unSelectedItemColor
						);
					}
	...
	private getTabBarItemAppearance(): MDCTabBarItemAppearance {
		...
		if (
			this.tabStrip &&
			this.tabStrip._hasImage &&
			this.tabStrip._hasTitle
		) {
	...
	private getIconRenderingMode(): UIImageRenderingMode {
		switch (this.tabStrip && this.tabStrip.iosIconRenderingMode) {
			...
			default:
				const hasItemColor =
					this._selectedItemColor || this._unSelectedItemColor;
	...
	private getIcon(tabStripItem: TabStripItem, color?: Color): UIImage {
		...
		if (!image) {
			...
			if (isFontIconURI(iconSource)) {
				...
				is = ImageSource.fromFontIconCodeSync(
					fontIconCode,
					font,
					color
				);
			...
			if (is && is.ios) {
				...
				const originalRenderedImage = image.imageWithRenderingMode(
					renderingMode
				);
	...
	private equalUIColor(first: UIColor, second: UIColor): boolean {
		...
		return (
			firstComponents[0] === secondComponents[0] &&
	...
	private isSelectedAndHightlightedItem(tabStripItem: TabStripItem): boolean {
		...
		return (
			tabStripItem._index === this.selectedIndex &&
			tabStripItem["_visualState"] === "highlighted"
		);
	}

	public setTabBarItemBackgroundColor(
		tabStripItem: TabStripItem,
		value: UIColor | Color
	): void {
		...
		const itemSelectedAndHighlighted = this.isSelectedAndHightlightedItem(
			tabStripItem
		);
		...
		if (
			this.viewController.tabBar.alignment !==
				MDCTabBarAlignment.Justified &&
			itemSelectedAndHighlighted &&
	...
	private setIconColor(
		...
		const tabStripColor =
			this.selectedIndex === tabStripItem._index
				? this._selectedItemColor
				: this._unSelectedItemColor;
	...
	public setTabBarFontInternal(value: Font): void {
		...
		const tabItemFontSize =
			this.tabStrip.style.fontSize || defaultTabItemFontSize;
		const font: UIFont = (
			this.tabStrip.style.fontInternal || Font.default
		).getUIFont(UIFont.systemFontOfSize(tabItemFontSize));
	...
	public setTabBarTextTransform(value: TextTransform): void {
		if (value === "none") {
			...
			this._ios.tabBar.titleTextTransform =
				MDCTabBarTextTransform.Uppercase;
		} else if (value === "initial") {
			this._ios.tabBar.titleTextTransform =
				MDCTabBarTextTransform.Automatic;
		}
	}
	...
	public setTabBarColor(value: UIColor | Color): void {
		...
		this._ios.tabBar.setTitleColorForState(
			nativeColor,
			MDCTabBarItemState.Normal
		);
		this._ios.tabBar.setTitleColorForState(
	...
	[selectedIndexProperty.setNative](value: number) {
		...
		if (value > -1) {
			...
			const controllers = NSMutableArray.alloc<
				UIViewController
			>().initWithCapacity(1);
			...
			this.visitFrames(
				item,
				(frame) => (frame._animationInProgress = true)
			);
			...
			this.viewController.setViewControllersDirectionAnimatedCompletion(
				...
				(finished: boolean) => {
					this.visitFrames(
						item,
						(frame) => (frame._animationInProgress = false)
					);
					if (finished) {
	...
	[iOSTabBarItemsAlignmentProperty.getDefault](): IOSTabBarItemsAlignment {
		...
		return <any>(
			(alignment.charAt(0).toLowerCase() + alignment.substring(1))
		);
	}

	[iOSTabBarItemsAlignmentProperty.setNative](
		value: IOSTabBarItemsAlignment
	) {
		if (!this.viewController || !this.viewController.tabBar) {
			return;
		}
	...
	private setViewTextAttributes(
		...
		const font: UIFont = (
			view.style.fontInternal || Font.default
		).getUIFont(UIFont.systemFontOfSize(tabItemFontSize));
		...
		const textColor =
			tabItemTextColor instanceof Color ? tabItemTextColor.ios : null;

		if (textColor) {
			this.viewController.tabBar.setTitleColorForState(
				textColor,
				MDCTabBarItemState.Normal
			);
			this.viewController.tabBar.setImageTintColorForState(
				textColor,
				MDCTabBarItemState.Normal
			);
